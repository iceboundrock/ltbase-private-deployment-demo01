...
在当前仓库版本中，deployment repo 也是 Control Plane UI 输入的操作者侧权威来源，例如 `CONTROLPLANE_UI_DOMAIN`、公开浏览器 auth 配置、auth provider 名称对齐，以及 Control Plane CORS 输入。由于部分 bootstrap 脚本仍会通过 companion 风格的 `*-controlplane-ui` 仓库发布这些值，下面的文档会保守地描述当前真实行为。
...
- 一个已被 Control Plane CORS 允许、并与操作者身份提供方 redirect URI 对齐的 control-plane UI 管理域名
- 每个需要提供操作者登录的 stack 都已经准备好浏览器可公开的 Firebase 和 Supabase Control Plane UI 配置值
- 至少一条操作者登录路径，其 provider 名称与该 stack 的 deployment-owned auth provider 记录保持一致
...
- Cloudflare 输入值已经准备好。
  ...
  - 如果你正在使用当前的 Control Plane UI companion bootstrap 流程，还要确认同一个 Cloudflare account 和 zone 也已经为 Control Plane UI 的 Pages project、自定义域名和所需 `CNAME` 做好准备。
  - 如果操作者账号或 token 不满足最小权限矩阵，请改用手动路径，而不是直接执行一键 bootstrap。
  - 确认该 zone 可以将 `api`、`auth`、`control-plane` 这些 hostname 通过 Cloudflare 代理出去。
  - 计划在承载正式流量前保持 Cloudflare SSL 模式为 `Full (strict)`，并启用 Authenticated Origin Pulls。
- Release 与应用相关输入已经准备好。
  - 确认 `LTBASE_RELEASES_REPO`、`LTBASE_RELEASE_ID`、`LTBASE_RELEASES_TOKEN` 和 `GEMINI_API_KEY` 在继续前都已经可用。
  - 确认当前 Control Plane UI 输入已经定稿，包括 `CONTROLPLANE_UI_DOMAIN`、`FIREBASE_API_KEY_<STACK>`、`FIREBASE_PROJECT_ID_<STACK>`、`SUPABASE_URL_<STACK>` 和 `SUPABASE_ANON_KEY_<STACK>`。
  - 确认这些 Firebase 和 Supabase 值都只是浏览器公开配置，不包含 secret、service-role key 或其他服务端凭据。
  - 确认每个 `infra/auth-providers.<stack>.json` 文件中的 provider 名称与操作者会看到的浏览器 provider 名称保持一致。当前 companion runtime config 生成逻辑会在渲染 `public/ltbase-controlplane.config.json` 时复用这些 deployment-owned provider 名称。
  - 确认首次管理员登录前，身份提供方已经允许 `https://<CONTROLPLANE_UI_DOMAIN>/auth/callback`，并且目标 LTBase project 已完成必要的管理员用户或用户组绑定。
- `.env` 内容是干净的。
  - 由你自己填写客户可控输入值。
...
- 内容包括：仅基础设施的 preview、按 promotion path 推进的 rollout、受保护环境审批、Control Plane UI 操作者侧校验，以及 managed DSQL 的 bootstrap 后处理
...
- 官方 deploy 工作流还会在 `pulumi up` 之后，基于部署出的 `projectId`、`apiId`、`apiBaseUrl` 和当前 AWS account id 自动回写 authservice 所需的 `project info` DynamoDB 记录
- 官方 deploy 工作流会通过 Pulumi 导出的 `dsqlClusterIdentifier` 从 AWS 获取权威 endpoint
- reconcile 步骤会把解析出的 endpoint 写入 stack config 的 `dsqlEndpoint`
- 同一条官方 rollout 路径随后会执行第二次 apply，让 Lambda 环境变量在该次 workflow 运行内拿到这个 managed endpoint
- `scripts/reconcile-managed-dsql-endpoint.sh` 仍然保留为恢复路径，用于在官方 workflow 之外修复已有 stack
...
- 从模板生成出的客户部署仓库不会发布预构建 infra binary；复制过去的 `build-infra-binary.yml` 在 `Lychee-Technology/ltbase-private-deployment` 之外应当始终被跳过
- 客户部署仓库现在会携带 `__ref__/template-provenance.json`，其中记录了用于查找预构建 infra binary 的上游模板 commit 和 `build_fingerprint`
- 只有当该 provenance 与 `build_fingerprint` 和上游已发布 manifest 完全匹配时，官方工作流才会安装预构建 binary；否则会按设计回退到源码构建
- 手动 preview 只支持 `PROMOTION_PATH` 中的第一个环境
- 受保护环境的 promotion 会在你自己的仓库中通过各自的 GitHub environment gate 完成
- 当前仓库仍然保留 companion 风格的 Control Plane UI bootstrap 和配置发布步骤，因此这份 runbook 刻意记录的是这些当前脚本行为，而不是一个尚未在这里完全验证的新 UI 部署模型

## Cloudflare mTLS 上线说明

- 当前模板默认要求 `api`、`auth`、`control-plane` 都通过 Cloudflare 代理的自定义域名提供访问。
- API Gateway 自定义域名会使用模板内置的 Cloudflare Authenticated Origin Pull truststore 开启 mutual TLS。
- 当前版本只文档化要求，不通过 IaC 自动切换 Cloudflare Authenticated Origin Pulls，因此操作者仍需在 Cloudflare 控制面中启用它。
- 上线后，直接访问 `https://<api-id>.execute-api.<region>.amazonaws.com/...` 失败是预期行为，因为默认 API Gateway endpoint 已被禁用。
- 自定义域名返回 `403`，通常表示 Cloudflare 没有呈现预期的客户端证书链，或者 truststore / 配置已经漂移。
- Cloudflare 返回 `526`，通常表示源站 TLS 与 `Full (strict)` 不兼容，或者自定义域名证书尚未有效。
- 建议上线顺序：
  - 先在低环境 apply 该基础设施变更
  - 在 Cloudflare 中确认 DNS 记录已经橙云代理
  - 确认 Authenticated Origin Pulls 已启用
  - 验证经 Cloudflare 的自定义域名访问成功
  - 验证直连 `execute-api` 访问失败
