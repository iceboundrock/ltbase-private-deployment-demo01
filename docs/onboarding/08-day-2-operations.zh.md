...
6. 运行 preview 工作流。
7. 审查 Pulumi preview 输出。记住 preview 只做基础设施预览，不会发布 Control Plane UI。
...
### 排查 Control Plane UI 操作者访问问题

当操作者无法登录 admin UI 时，在修改 workflow 或 release 输入前，先检查以下事项：

- `CONTROLPLANE_UI_DOMAIN` 仍然指向预期的 Cloudflare Pages 自定义域名和 DNS `CNAME`
- 身份提供方仍允许 `https://<CONTROLPLANE_UI_DOMAIN>/auth/callback`
- `infra/auth-providers.<stack>.json` 里仍存在与当前 companion runtime config 中渲染出来的浏览器 provider 名称一致的 provider 记录
- 除非你明确配置成 `*`，否则 Control Plane API 的 CORS allowlist 仍包含 `https://<CONTROLPLANE_UI_DOMAIN>`
- `.env` 中的 Firebase 和 Supabase 值仍然是浏览器公开值，并且与你预期的操作者侧应用设置保持一致
- 当前仓库版本仍可能依赖 companion 风格的 Control Plane UI bootstrap 假设，因此在把问题视为应用回归前，先确认这些 companion 资源仍然存在

## 运维提醒

- 不要在部署仓库中自行重建 LTBase 应用二进制
- 不要在部署仓库中自行重建 LTBase Control Plane UI 源码产物
- 不要提交 `.env`
- 不要绕过生产审批 gate
